...
const DoctorantConferences = () => {
  ...
  const removeConference = (id) => {
    setConferences(prev => prev.filter(conf => conf.id !== id));
  };

  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-4 text-gray-800">Mes Conférences</h1>
      <p className="mb-6 text-gray-600">
        Déclarez ici vos communications faites lors de conférences nationales ou internationales.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-lg shadow-md">
        <div>
          ...
          <input
            ...
            className={`w-full border rounded p-2 ${errors.conferenceName ? "border-red-500" : "border-gray-300"}`}
            placeholder="Ex: Conférence IA Maroc 2024"
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className={`w-full border rounded p-2 ${errors.locationDate ? "border-red-500" : "border-gray-300"}`}
            placeholder="Ex: Rabat, Juin 2024"
          />
          {errors.locationDate && <p className="text-red-500 text-sm mt-1">{errors.locationDate}</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className={`w-full border rounded p-2 ${errors.articleTitle ? "border-red-500" : "border-gray-300"}`}
            placeholder="Titre de votre présentation"
          />
          {errors.articleTitle && <p className="text-red-500 text-sm mt-1">{errors.articleTitle}</p>}
        </div>
        ...
        <div>
          ...
          <div className="flex items-center">
            <label className="cursor-pointer bg-gray-100 hover:bg-gray-200 px-4 py-2 rounded border border-gray-300">
              <span>{newConference.fileName || "Choisir un fichier"}</span>
              <input
                type="file"
            ...
            {newConference.fileName && (
              <button
                type="button"
                onClick={() => setNewConference(prev => ({ ...prev, file: null, fileName: "" }))}
      ...
      {conferences.length > 0 && (
        <div className="mt-8">
          <h2 className="text-xl font-semibold mb-4 text-gray-800">Conférences enregistrées</h2>
          <div className="space-y-4">
            {conferences.map((conference) => (
              <div key={conference.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-medium text-gray-800">{conference.conferenceName}</h3>
                    <p className="text-sm text-gray-500">{conference.locationDate}</p>
                    <p className="mt-1 text-gray-600">{conference.articleTitle}</p>
                  </div>
                  <button
                    onClick={() => removeConference(conference.id)}
                    className="text-red-500 hover:text-red-700"
                    aria-label="Supprimer cette conférence"
                  >
                ...
                {conference.fileName && (
                  <div className="mt-2">
                    <span className="text-sm text-gray-500">Fichier joint: {conference.fileName}</span>
                  </div>
                )}
              </div>
            ))}
