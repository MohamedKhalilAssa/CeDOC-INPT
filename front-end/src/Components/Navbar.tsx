      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          <div className="flex items-center">
            ...
            <span className="ml-3 text-xl font-bold hidden md:block bg-gradient-to-r from-blue-800 via-blue-600 to-blue-500 bg-clip-text text-transparent">CEDoc</span>
          </div>
          <div className="hidden lg:flex space-x-6 ml-12">
            {['accueil', 'programmes', 'recherche', 'admission', 'contact'].map(section => (
              <a
                key={section}
                href={`#${section}`}
          ...
          <div className="lg:hidden">
            ...
            >
              <svg
                className={`h-8 w-8 transform transition duration-300 ${menuOpen ? 'rotate-90' : ''}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
      ...
      {menuOpen && (
        <div className="lg:hidden bg-white shadow-lg">
          <div className="px-2 pt-2 pb-3 space-y-2 sm:px-3">
            {['accueil', 'programmes', 'recherche', 'admission', 'contact', 'candidature'].map(section => (
              <a
                key={section}
                href={`#${section}`}
