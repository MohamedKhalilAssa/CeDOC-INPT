...
public class EmailVerificationService {
  ...
  public CompletableFuture<Void> sendVerificationToken(String email) {
    final Utilisateur user = utilisateurRepository.findByEmail(email).filter(u -> !u.isEmailValider())
        .orElseThrow(() -> new ResponseStatusException(
            org.springframework.http.HttpStatus.BAD_REQUEST, "Email introuvable ou déjà vérifié"));
    ...
    try {
          ...
          <html>
            <body style='margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f4f4f4;'>
              <div style='max-width: 700px; margin: 20px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);'>
                ...
                <div style='background-color: #fff;border-bottom: 1px solid #333; padding: 20px; text-align: center;'>
                ...
                <div style='padding: 30px;'>
                  ...
                  <p style='font-size: 16px; color: #555;'>Merci de vous être inscrit. Pour activer votre compte, veuillez confirmer votre adresse email en cliquant sur le bouton ci-dessous :</p>

                  <div style='text-align: center; margin: 30px 0;'>
                    <a href='%s' style='background-color: #007BFF; color: white; padding: 12px 25px; border-radius: 5px; text-decoration: none; font-size: 16px;'>Vérifier mon email</a>
                  </div>

                  <hr style='border: none; border-top: 1px solid #eee;' />

                  <p style='font-size: 14px; color: #666;'>Si le bouton ne fonctionne pas, vous pouvez utiliser ce code :</p>
                  <div style='text-align: center; margin: 20px 0;'>
                    <code style='font-size: 20px; font-weight: bold; color: #222;'>%s</code>
                  </div>

                  <p style='font-size: 13px; color: #999;'>Ce lien/code expirera dans 15 minutes.</p>
                  <p style='font-size: 13px; color: #999;'>Si vous n'avez pas demandé cette vérification, ignorez ce message.</p>
                </div>
      ...
      String content = String.format(template, logoUrl, fullName, verificationUrl, token);
  ...
  @Transactional
  public Utilisateur verifyEmail(String email, String token) {
